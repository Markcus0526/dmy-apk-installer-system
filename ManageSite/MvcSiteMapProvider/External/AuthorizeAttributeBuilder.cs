...
namespace Telerik.Web.Mvc.Infrastructure.Implementation
{
    ...
    public class AuthorizeAttributeBuilder
    {
        ...
        private static void WriteProperty(Type parentType, TypeBuilder builder, string name, Type type)
        {
            ...
            var parentGetMethod = parentType.GetMethod(getName, BindingFlags.Public | BindingFlags.Instance);
            var implementedGetMethod = builder.DefineMethod(getName, MethodAttributes.Public | MethodAttributes.Virtual, type, Type.EmptyTypes);
            var getIl = implementedGetMethod.GetILGenerator();
            getIl.Emit(OpCodes.Ldarg_0);
            getIl.Emit(OpCodes.Call, parentGetMethod);
            getIl.Emit(OpCodes.Ret);

            var interfaceGetMethod = AuthorizeAttributeType.GetMethod(getName, BindingFlags.Public | BindingFlags.Instance);
            builder.DefineMethodOverride(implementedGetMethod, interfaceGetMethod);

            var parentSetMethod = parentType.GetMethod(setName, BindingFlags.Public | BindingFlags.Instance);
            var implementedSetMethod = builder.DefineMethod(setName, MethodAttributes.Public | MethodAttributes.Virtual, typeof(void), new[] { type });
            var setIl = implementedSetMethod.GetILGenerator();
            setIl.Emit(OpCodes.Ldarg_0);
            setIl.Emit(OpCodes.Ldarg_1);
            setIl.Emit(OpCodes.Call, parentSetMethod);
            setIl.Emit(OpCodes.Ret);

            var interfaceSetMethod = AuthorizeAttributeType.GetMethod(setName, BindingFlags.Public | BindingFlags.Instance);
