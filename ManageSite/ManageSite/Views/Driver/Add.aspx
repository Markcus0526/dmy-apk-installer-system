...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
	...
	<div class="container-fluid">
		...
		<div class="row-fluid">
			<div class="span12">
				<!-- BEGIN FORM-->
                <form id="driverfileform" name="driverfileform" action="<%= ViewData["rootUri"] %>Upload/UploadFile" method="post" enctype="multipart/form-data">
                    <input type="file" id="uploadfile" name="uploadfile" onchange="showSelectedFile();" style="display:none;" />
                </form>
				<form action="<%= ViewData["rootUri"] %>Driver/Add" method="post" id="form_edit" name="form_edit" class="form-horizontal">
					...
					<div class="control-group">
						<label class="control-label" for="name">手机类型<span class="required">*</span></label>
						<div class="controls">
							<select id="telid" name="telid" class="span6 select2">
                            <% foreach (PhoneInfo item in (List<PhoneInfo>)ViewData["tellist"])
                               { %>
								<option value="<%= item.uid %>" <% if (ViewData["telid"] != null && ViewData["telid"].ToString() == item.uid.ToString()) { %>selected <% } %>><%= item.vendor %>-<%= item.type %>
                                </option>
                            <% } %>
                            </select>
						</div>
					...
					<div class="control-group">
						<label class="control-label" for="name">驱动名称<span class="required">*</span></label>
						<div class="controls">
							<input type="text" name="name" id="name" placeholder="请输入驱动名称" data-required="1" class="span6 m-wrap" 
                                value="<% if (ViewData["name"] != null) { %><%= ViewData["name"] %><% } %>"/>
						</div>
					</div>
					<div class="control-group">
						<label class="control-label" for="name">驱动文件<span class="required">*</span></label>
						<div class="controls">
                            <input type="hidden" id="selecteddriver" name="selecteddriver" value="<% if (ViewData["filepath"] != null) { %><%= ViewData["filepath"] %><% } %>" />
                            <input type="hidden" id="filepath" name="filepath" value="<% if (ViewData["filepath"] != null) { %><%= ViewData["filepath"] %><% } %>" />
                            <input type=button class="btn btn-small btn-primary" id='driverfilebtn' value="选择驱动文件">
					...
					<div class="control-group">
						<label class="control-label" for="sysver">版本<span class="required">*</span></label>
						<div class="controls">
							<input type="text" name="sysver" id="sysver" placeholder="请输入版本" data-required="1" class="span6 m-wrap" 
					...
					<div class="form-actions">
						...
						<a href="<%= ViewData["rootUri"] %>Driver/List" class="btn">取消</a>
					</div>
...
<% } %>
	        ...
	        form1.validate({
	            ...
	            submitHandler: function (form) {
                    ...
                    if ( $('#uploadfile').val() != "") {
                        $("form[name=driverfileform]").submit();
                    } else {
        ...
        var initFormUpload = function() {
            ...
            $('form[name=driverfileform]').ajaxForm({
                beforeSend: function () {
                    status.empty();
        ...
        function showSelectedFile()
        {
            ...
            $('#selecteddriver').attr("value", $('#uploadfile').val());
        }
    </script>
</asp:Content>
