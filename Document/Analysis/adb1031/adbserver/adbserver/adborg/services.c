...
void restart_root_service(int fd, void *cookie)
{
    ...
    if (getuid() == 0) {
        _snprintf(buf, sizeof(buf), "adbd is already running as root\n");
        writex(fd, buf, strlen(buf));
        adb_close(fd);
    } else {
        property_get("ro.debuggable", value, "");
        if (strcmp(value, "1") != 0) {
            _snprintf(buf, sizeof(buf), "adbd cannot run as root in production builds\n");
            writex(fd, buf, strlen(buf));
            adb_close(fd);
            return;
        }
        ...
        _snprintf(buf, sizeof(buf), "restarting adbd as root\n");
        writex(fd, buf, strlen(buf));
        adb_close(fd);
    }
}
...
void restart_tcp_service(int fd, void *cookie)
{
    ...
    if (port <= 0) {
        _snprintf(buf, sizeof(buf), "invalid port\n");
        writex(fd, buf, strlen(buf));
        adb_close(fd);
        return;
    }

    _snprintf(value, sizeof(value), "%d", port);
    property_set("service.adb.tcp.port", value);
    _snprintf(buf, sizeof(buf), "restarting in TCP mode port: %d\n", port);
    writex(fd, buf, strlen(buf));
    adb_close(fd);
}
...
void restart_usb_service(int fd, void *cookie)
{
    ...
    _snprintf(buf, sizeof(buf), "restarting in USB mode\n");
    writex(fd, buf, strlen(buf));
    adb_close(fd);
}
...
void reboot_service(int fd, void *arg)
{
    ...
    if (ret < 0) {
        _snprintf(buf, sizeof(buf), "reboot failed: %s\n", strerror(errno));
        writex(fd, buf, strlen(buf));
    }
    free(arg);
    adb_close(fd);
...
static int create_subprocess(const char *cmd, const char *arg0, const char *arg1, pid_t *pid)
{
    ...
    *pid = fork();
    ...
    if(*pid == 0){
        ...
        char text[64];
        _snprintf(text, sizeof text, "/proc/%d/oom_adj", getpid());
