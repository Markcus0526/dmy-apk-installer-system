...
#if 0 //ndef HAVE_WIN32_IPC  /* no Unix-domain sockets on Win32 */
...
int launch_server(int server_port)
{
	...
	_splitpath(program_path, drive, dir, fname, ext);

	_makepath(program_path, drive, dir, "aiadb", "exe");

	/* Add by CSC: Non-block pipe connection**********/
	hProcess = SpawnAndRedirect(program_path, &hOutput, NULL);
	if (!hProcess) return;

	ret = ReadFile( hOutput, temp, 3, &count, NULL );
	CloseHandle(hOutput);
...
int handle_host_request(char *service, transport_type ttype, char* serial, int reply_fd, asocket *s)
{
    ...
    char buf[4096];
	if ( !strcmp(service, "killadb") || !strcmp(service, "kill"))
	{
    ...
    if(!strcmp(service,"list-forward")) {
        ...
        _snprintf(header, sizeof header, "OKAY%04x", buffer_size);
