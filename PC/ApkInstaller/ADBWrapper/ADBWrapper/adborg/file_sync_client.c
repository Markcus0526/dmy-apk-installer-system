...
static int write_data_file(adb_client_info * client, int fd, const char *path, syncsendbuf *sbuf)
{
    int lfd, err = 0;
...
static int write_data_buffer(adb_client_info * client, int fd, char* file_buffer, int size, syncsendbuf *sbuf)
{
    int err = 0;
    int total = 0;
    ...
    while (total < size) {
		...
		{
			char str[64];
			//Comment by CSC: no need to every progress, because it is very dangerous on pipe communication.(Some devices are slow to repond) 
			//_snprintf(str, 64,"Push%.2f", total/(size+0.0001f));
			//report_to_cs(client, str);
		}
    }
...
static int sync_send(adb_client_info * client, int fd, const char *lpath, const char *rpath,
                     unsigned mtime, mode_t mode, int verifyApk)
{
    syncmsg msg;
    int len, r;
    syncsendbuf *sbuf = &(client->send_buffer);
    ...
    if (file_buffer) {
        write_data_buffer(client, fd, file_buffer, size, sbuf);
        free(file_buffer);
    } else if (S_ISREG(mode))
        write_data_file(client, fd, lpath, sbuf);
#ifdef HAVE_SYMLINKS
...
int sync_recv(adb_client_info* client, int fd, const char *rpath, const char *lpath)
{
    syncmsg msg;
    int len;
    int lfd = -1;
    char *buffer = client->send_buffer.data;
    unsigned id;
...
int do_sync_ls(adb_client_info* client, const char *path)
{
    int fd = adb_connect(client, "sync:");
    if(fd < 0) {
...
static int copy_local_dir_remote(adb_client_info * client, int fd, const char *lpath, const char *rpath, int checktimestamps, int listonly)
{
    copyinfo *filelist = 0;
    copyinfo *ci, *next;
    int pushed = 0;
    ...
    for(ci = filelist; ci != 0; ci = next) {
        ...
        if(ci->flag == 0) {
            ...
            if(!listonly &&
               sync_send(client, fd, ci->src, ci->dst, ci->time, ci->mode, 0 /* no verify APK */)){
                return 1;
            }
            pushed++;
        } else {
...
int do_sync_push(adb_client_info * client, const char *lpath, const char *rpath, int verifyApk)
{
    struct stat st;
    unsigned mode;
    int fd;

    fd = adb_connect(client, "sync:");
    if(fd < 0) {
        fprintf(stderr,"error: %s\n", adb_error(client));
        return 1;
    }
    ...
    if(S_ISDIR(st.st_mode)) {
        BEGIN();
        if(copy_local_dir_remote(client, fd, lpath, rpath, 0, 0)) {
            return 1;
        } else {
            END();
            sync_quit(fd);
        ...
        BEGIN();
        if(sync_send(client, fd, lpath, rpath, st.st_mtime, st.st_mode, verifyApk)) {
            return 1;
        } else {
            END();
            sync_quit(fd);
...
static int copy_remote_dir_local(adb_client_info * client, int fd, const char *rpath, const char *lpath,
                                 int checktimestamps)
{
    copyinfo *filelist = 0;
    copyinfo *ci, *next;
    ...
    for (ci = filelist; ci != 0; ci = next) {
        ...
        if (ci->flag == 0) {
            ...
            if (sync_recv(client, fd, ci->src, ci->dst)) {
                return 1;
            }
            pulled++;
        } else {
...
int do_sync_pull(adb_client_info * client, const char *rpath, const char *lpath)
{
    unsigned mode;
    struct stat st;
    ...
    if(S_ISREG(mode) || S_ISLNK(mode) || S_ISCHR(mode) || S_ISBLK(mode)) {
        ...
        BEGIN();
        if(sync_recv(client, fd, rpath, lpath)) {
            return 1;
        } else {
            END();
            sync_quit(fd);
        ...
        BEGIN();
        if (copy_remote_dir_local(client, fd, rpath, lpath, 0)) {
            return 1;
        } else {
            END();
            sync_quit(fd);
...
int do_sync_sync(adb_client_info * client, const char *lpath, const char *rpath, int listonly)
{
	int fd;
    ...
    BEGIN();
    if(copy_local_dir_remote(client, fd, lpath, rpath, 1, listonly)){
        return 1;
    } else {
        END();
        sync_quit(fd);
