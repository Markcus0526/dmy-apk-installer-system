...
wchar_t test_interface_name[] = L"\\\\?\\usb#vid_18d1&pid_dddd#123456789abcdef#{F72FE0D4-CBCB-407d-8814-9ED673D0DD6B}";
...
int _tmain(int argc, _TCHAR* argv[]) {
  ...
  hAdvWinApi = LoadLibraryA("AdbWinApi.dll");
  // General startup tests.
  //RunInterfaceEnumTest1();
  argc -= 2;
  argv += 2;
  ...
  if (argv[0][0] =='-' && argv[0][1]=='s') {
	  if (isdigit(argv[0][2])) {
		  ...
		  if(argc < 2 || argv[0][2] != '\0') return 0;
		  serial = argv[1];
		  argc--;
		  argv++;
	  }
  }
  install_app(kTransportAny, serial, argc, argv);
//	InstallApk();
	return 0;
}
...
bool RunInterfaceEnumTest(bool exclude_not_present,
  ...
  do {
    ...
    if (ERROR_INSUFFICIENT_BUFFER == GetLastError()) {
      ...
      if (res) {
        if (
			exclude_not_present && active_only &&
